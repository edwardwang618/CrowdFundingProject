// 声明函数封装导航条初始化操作
function initPagination(totalRecord, pageSize, pageNum) {
...
function pageselectCallback(pageIndex, jq) {
    ...
    // 跳转页面
    window.location.href = "admin/query/for/search.html?pageNum=" + pageNum + "&keyword=" + window.keyword;

    return false;
}

// 封装执行批量删除的函数
function doBatchRemove(adminIdArray, pageNum, keyword) {
    ...
    $.ajax({
        ...
        "success":function(response) {		// 服务器处理请求成功后执行的函数，响应体以参数形式传入当前函数
            ...
            if (result === "SUCCESS") {
                window.location.href = "admin/query/for/search.html?pageNum=" + pageNum + "&keyword=" + keyword;
            }
